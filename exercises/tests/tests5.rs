...
unsafe fn modify_by_address(address: usize) {
    ...
    //# Safety: The `address` must contain a mutable reference to a valid `u32` value.
    // The `address` is guaranteed to be valid and contains a unique reference to a `u32` local variable.
    // The `unsafe` block ensures that the reference is valid and unique, and the `todo!` macro
    // is used to indicate the remaining work to be done.
    unsafe {
